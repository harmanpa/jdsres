 ...
 * This class provides some helpful utilities for reading data and finding
 * files.
 *
 * @author Eider Moore
 */
public class FileUtilities {
    ...
    /**
     * Use a base URL to find every file with the same name and extensions from
     * the list. If a url doesn't exist, it is left out of the result.
     *
     * @param url
     * @param extensions
     * @return A set of URI's referencing files with the same path/name as url
     * and extensions from the list.
    ...
    /**
     * Use a base File to find every file with the same name and extensions from
     * the list. If a url doesn't exist, it is left out of the result.
     *
     * @param file
     * @param extensions
    ...
    public static URI[] findURIs(File file, String[] extensions) {
        ...
        ArrayList<URI> answer = new ArrayList<>(extensions.length);
        for (String ext : extensions) {
     ...
     * @return A set of URI's referencing files with the same path/name as url
     * and extensions from the list.
     * @throws URISyntaxException If we cannot convert the url to a uri. This
     * won't really be thrown, but it is backwards compatible.
     * @throws IOException if the given url doesn't exist
     */
    public static URL[] findURLs(URL url, String[] extensions) throws URISyntaxException, IOException {
        try {
            ...
            ArrayList<URL> answer = new ArrayList<>(extensions.length);
            for (String ext : extensions) {
                boolean exists;
                URL cur;
                try {
     ...
     * improperly configured servers). In the end, it will not be an error to
     * read from this url (assuming the server or file don't go down).
     *
     * @param url
    ...
    /**
     * Read an input stream into bytes. This supports automatic decompression.
     * If your file may legitimately begin with the magic number (2 bytes: [31,
     * 139]) and you know it isn't gzipped, set supportedCompression null (also
     * set it to null if you already have the proper stream).<br>
     * <b> The stream is closed at the end of this method</b>
     *
     * @param in The input stream
     * @param estimatedSize an estimated size. this is more efficient if
     * accurate.
     * @param supportedCompression An enum set of compression types or null for
     * none
     ...
     * Read an input stream into bytes. This supports automatic decompression.
     * If your file may legitimately begin with the magic number (2 bytes: [31,
     * 139]) and you know it isn't gzipped, set supportedCompression to null
     * (also set it to null if you already have the proper stream).<br>
    ...
    /**
     * Check that in is buffered and check if gzip is supported, return the
     * proper stream that is buffered or decodes the compression.
     *
     * @param in
     * @param supportedCompression An enum set of compression types or null for
     * none
    ...
    /**
     * Mac started generating illegal jar urls for resources, this is an
